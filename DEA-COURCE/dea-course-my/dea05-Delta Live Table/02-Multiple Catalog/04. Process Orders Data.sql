...
FROM cloud_files ('/Volumes/circuitbox-new/landing/operational_data/orders/',
                  'json',
...
SELECT order_id,
       ...
       items,
       order_status
FROM STREAM (LIVE.bronze.orders);
...
CREATE STREAMING TABLE silver.orders
AS
SELECT order_id,
       customer_id,
       order_timestamp,
...
FROM (SELECT order_id,
      ...
      FROM STREAM(LIVE.silver.orders_clean));
